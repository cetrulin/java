...
public class Genetico {
	...
	public static void cruzar () {
		...
		for(int i = indexReina.length-1,cuentaRep=0;cuentaRep<numeroReproducciones&&i>0;i-=2,cuentaRep++){
			...
			if(indexFitness[i]>0||indexFitness[i-1]>0){
				...
				indexFitness [i] = calculaColisiones (indexReina[i]);
				indexFitness [i-1] = calculaColisiones (indexReina[i-1]);
			}
	...
	/*public static int[] mutar (int [] hijo) {
		//elige aleatoriamente que reinas cambiara y las cambia a una posicion aleatoria
		int rand = (int)(Math.random()*(n));
		hijo[rand] = (int)(Math.random()*(n));
		return hijo;
	}*/
